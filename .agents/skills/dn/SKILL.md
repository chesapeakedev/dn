...
[docs/subcommands.md](../../../docs/subcommands.md). The shared outer-harness
contract is [docs/outer-harness.md](../../../docs/outer-harness.md).
...
- After a **local** (`--publish none`) run, **ask before committing**. You write
  the commit if they say yes. Do not auto-run `dn land`. After `--publish pr` or
  `--publish direct`, kickstart already published — report the PR or branch.
