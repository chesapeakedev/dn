...
creates a conventional-commit message (preferring one commit). Use `single=true`
for one deterministic commit. Use `dryRun=true` to preview without committing.
Use `issueTestplan=true` to upsert a `## Test Plan` onto the linked GitHub issue
before committing.
...
- `CLAUDE_ENABLED` - Set to `1` to use Claude Code by default (do not combine
  with another agent env toggle)
- `CODEX_ENABLED` - Set to `1` to use Codex CLI by default (do not combine with
  another agent env toggle)
