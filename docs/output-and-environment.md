...
| **EDITOR**           | In attended runs, opens **agent-generated** drafts for review before the next step: kickstart plans, meld GitHub issue/comment bodies, `meld --update-issue`, and `land --issue-testplan` upserts. Supports command arguments such as `code --wait`. Caller-supplied `--body-file` / `--body-stdin` is never opened. |
