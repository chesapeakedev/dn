...
### Continuation flow (next-issue chaining)

After every successful kickstart run the CLI enters a continuation loop that can
chain to the next queued item. The loop uses one of two queue backends and its
behavior depends on flags and what comes next.

#### Queue backends

**Todo list** (`~/.dn/todo.md`) — the default when no `--milestone` is set.
The file is a prioritized checklist of issues and plan paths managed by
`dn todo`, `dn tidy`, and `dn peek`. On start without an explicit ticket
the CLI reads this file and prompts `"Proceed with <ref>?"` for the first
unchecked item. If the list is empty the CLI offers to search the repo for open
issues, score them via the LLM, write the result, and prompt the first
suggestion.

**Milestone stack** (`plans/{owner}_{repo}_{num}.stack.md`) — activated by
`--milestone <num-or-url>`. The stack file is a checklisted set of issues
linked to a GitHub milestone, generated by `dn init stack`. On start the CLI
reads the file and picks the first unchecked task. See `dn init stack --help`.

#### Post-run loop

After a successful plan + implement cycle:

1. **Mark done** — the queue item is checked off in its backend. If the item is
   a GitHub issue, it is closed with a completion comment.
2. **Re-read queue** — the backlog is read fresh to discover any changes made
   during the run (e.g. new stack items added by a teammate).
3. **Next item** — if another unchecked item exists the user is prompted
   `"Proceed with <next.ref>?"` (y/n). Answering no exits.
4. **Empty queue** — if no more items remain the CLI prints a summary message
   and exits with a `dn land` hint when changes are uncommitted.

#### Flag-controlled modes

| Invocation | Prompt behavior | Use case |
|---|---|---|
| `dn kickstart` (no ticket, todo has items) | Prompt "Proceed?" before run, prompt "Continue?" after | Interactive single-issue or ad-hoc chaining |
| `dn kickstart --milestone 42` | Prompt "Proceed?" before each, prompt "Continue?" after | Interactive milestone walkthrough |
| `dn kickstart --milestone 42 --complete` | No prompts — auto-chain all remaining items | Batch milestone completion |
| `dn kickstart --milestone 42 --once` | No prompts — run exactly one item then exit | CI / one-shot automation |
| `dn kickstart <url>` (explicit ticket, no queue) | No queue interaction — run once, no chain | One-off issue |

`--complete` and `--once` are mutually exclusive and require `--milestone`.

#### Land interaction

When publish mode is `none` (default, local-only) the CLI suggests
`dn land` after the run. In multi-issue milestone sessions (`--complete` or
interactive chain) each issue's changes accumulate as pending work —
`dn land` is only suggested at the end. There is no automatic `dn land`
between chained items; the user is expected to run it explicitly when ready.

#### Exit conditions

The loop exits and the CLI returns to the shell when any of the following
are true:

- The user answers no to "Mark done and continue with next?"
- The user answers no to "Proceed with <next.ref>?"
- The queue has no more unchecked items (stack or todo)
- `--once` was set and the single item completed
- A run fails (orchestrator error) — the loop breaks immediately
